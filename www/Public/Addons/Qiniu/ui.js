...
function FileProgress(file, targetID) {
    ...
    if (!this.fileProgressWrapper.length) {
        ...
        var progressCancelTd = $("<td/>");
        var progressCancelBtn = $("<button/>");
        
        progressBarWrapper.addClass("progress progress-striped");
        progressCancelBtn.addClass("cancel_row btn btn-danger btn-sm")
                .attr('type', 'button')
                .html("取消");
        
        progressCancelTd.append(progressCancelBtn);
        ...
        // var progressOp = $("<td/>");
        // var cancelButton = $("<a href='javascript:void(0);' class='cancel_row btn btn-danger btn-sm'>取消</a>")
        // progressOp.append(cancelButton);

        Wrappeer.append(progressText);
        Wrappeer.append(progressSize);
        Wrappeer.append(progressBarTd);
        Wrappeer.append(progressCancelTd);
        // Wrappeer.append(progressOp);
...
FileProgress.prototype.setChunkProgess = function(chunk_size) {
    ...
    var viewProgess = $('<button class="btn btn-default btn-xs" type="button">查看分块上传进度</button>');
...
FileProgress.prototype.setComplete = function(up, info) {
    ...
    td.parents('tr').append("<td><a href='javascript:void(0);' class='delete_row btn btn-danger btn-sm'>删除</a></td>");

    var progressNameTd = this.fileProgressWrapper.find('.progressName');

    var Wrapper = $('<div class="Wrapper"/>');
    var playBtn = $('<input class="origin-video btn  btn-primary btn-xs play-btn" type="button" value="播放">');
