...
class SyncLoginModel extends \Gy_Library\GyModel {
//登录成功，获取腾讯QQ用户信息
    ...
    public function bindUser($uid, $openid, $type, $access_token, $refresh_token){
        //检查是否已绑定其他账户
        $map['openid'] = $openid;
        $map['type'] = $type;
        ...
        if($ent && $ent['uid'] != $uid){
            $this->error = '该账号已绑定其他用户';
            return false;
        }

        //清除账户该类型下的所有绑定记录，保证一个类型的社交账号只能绑定一个用户
        $this->where(array('type' => $type, 'uid' => $uid))->delete();
