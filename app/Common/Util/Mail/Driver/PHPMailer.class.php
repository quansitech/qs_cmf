...
namespace Common\Util\Mail\Driver;
 ...
 */
  ...
  */
  ...
  //0:host 1:port 2:char_set 3:encoding 4:mail_username 5:mail_password 6:from_mail
  public function init($param){
      $this->IsSMTP();
      
      $this->Host = $param[0]; //SMTP服务器 以163邮箱为例子
      $this->Port = $param[1];  //邮件发送端口
      $this->SMTPAuth   = true;  //启用SMTP认证

      $this->CharSet  = $param[2]; //字符集
      $this->Encoding = $param[3]; //编码方式
      $this->Username = $param[4];
  ...
  public function sendMail($param){
      ...
      $this->FromName = $param[0];
      
      $this->setRecipents('to', $param[1]);
      if($param[2]){
        $this->setRecipents('cc', $param[2]);
      }
      if($param[3]){
        $this->setRecipents('bcc', $param[3]);
      }
      $this->IsHTML(true);
      ...
      if($param[6]){
          $this->AddAttachment($param[6]['path'], $param[6]['name']);
      }
  ...
  //$kind 参数有 to,  cc,  bcc
  //$recipients 有两种格式，1种字符串 mail:name mail1:name1 mail2:name2  第二种是数组 array(array(mail, name), array(mail1, name1)) 或者 array(mail, mail1, mail2)
  //返回true 或者 false  false代表出错
  protected function setRecipents($kind, $recipients){
      $function = '';
      ...
      if(is_string($recipients)){
          $recipients .= ',';
          while(preg_match('/(.+?),/si', $recipients, $m)){ 
                $return = false;
                $args = array();
                if(preg_match('/^([a-zA-Z\d@\.\-_]+):(.+)/si', $m[1], $m_m)){
                    $args = array($m_m[1], $m_m[2]);
                ...
                $recipients = str_replace($m[0], '', $recipients);
          }
          return true;
      }
      ...
      if(is_array($recipients) ){
          foreach($recipients as $recipient){
              ...
              else if(is_array($recipient) && count($recipient) == 2){
                  $args = $recipient;
              }
              //获取参数出错
              if(count($args) == 0){
