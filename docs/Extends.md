...
>
> 8. registerHeadJs
>> + 说明: 若html存在 *** C('QS_REGISTER_JS_TAG_END') *** 标识，则注入js连接，自定义html字符串可以使用系统常量 *** __QS_REGISTER_JS_TAG_END__ *** 获取该值
>> + 参数: 
>> + src js连接地址
...
>
> 10. registerHeadCss
>> + 说明: 若html存在 *** C('QS_REGISTER_CSS_TAG_END') *** 标识，则注入css连接，自定义html字符串可以使用系统常量 *** __QS_REGISTER_CSS_TAG_END__ *** 获取该值
>> + 参数: 
>> + src css连接地址
